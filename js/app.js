var calculadora = {
	...
	LCD: document.getElementById("display"),
	valorLCD: "0",
	operacion: "",
	primerValor: 0,
	segundoValor: 0,
	...
	cambiarSigno: function(){
		if (this.valorLCD !="0") {
			var aux;
			if (this.valorLCD.charAt(0)=="-") {
				aux = this.valorLCD.slice(1);
			}	else {
				aux = "-" + this.valorLCD;
			}
		this.valorLCD = "";
	...
	ingresoDecimal: function(){
		if (this.valorLCD.indexOf(".")== -1) {
			if (this.valorLCD == ""){
				this.valorLCD = this.valorLCD + "0.";
			} else {
				this.valorLCD = this.valorLCD + ".";
			}
			this.updateLCD();
		}
	},
	...
	ingresoNumero: function(valor){
		if (this.valorLCD.length < 8) {
		
			if (this.valorLCD=="0") {
				this.valorLCD = "";
				this.valorLCD = this.valorLCD + valor;
			} else {
				this.valorLCD = this.valorLCD + valor;
			}
		this.updateLCD();
	...
	ingresoOperacion: function(oper){
		this.primerValor = parseFloat(this.valorLCD);
		this.valorLCD = "";
		this.operacion = oper;
		this.auxTeclaIgual = false;
		this.updateLCD();
	...
	verResultado: function(){ // TECLA IGUAL
		...
		if(!this.auxTeclaIgual){ //Primer vez que presiono igual
			this.segundoValor = parseFloat(this.valorLCD);
		...
		//Borro el LCD y lo reemplazo por el resultado
		this.valorLCD = "";
	
		//verifico el largo del resultado para recortarlo si hace falta

		if (this.resultado.toString().length < 8){
			this.valorLCD = this.resultado.toString();
		} else {
			this.valorLCD = this.resultado.toString().slice(0,7) + "...";
		}
	...
	updateLCD: function(){
		this.LCD.innerHTML = this.valorLCD;
	}
